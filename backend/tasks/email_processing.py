...
celery_app.conf.update(
    ...
    enable_utc=True,
    task_routes={
        'tasks.email_processing.process_email_message': {'queue': 'email_processing'},
    },
...
@celery_app.task(bind=True, name='tasks.email_processing.process_email_message')
def process_email_message(self, normalized_message_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"📧 Processing email message: {normalized_message.message_uuid}")
        ...
        if not has_booking_intent:
            print(f"📝 Email message without booking intent, skipping: {normalized_message.message_uuid}")
        ...
        try:
            ...
            print(f"✅ Email message processing completed: {normalized_message.message_uuid}")
            ...
            print(f"❌ Error in lead processing for email message {normalized_message.message_uuid}: {processing_error}")
        ...
        print(f"❌ Fatal error processing email message: {e}")
...
def health_check() -> Dict[str, Any]:
    """
    Perform health check for email processing Celery worker.
    ...
    try:
        ...
        # This would normally fail without proper email payload structure,
        # but we can at least test the MessageBus initialization
        print("Email processing health check passed")
